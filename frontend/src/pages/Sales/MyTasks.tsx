...
import { Target, TrendingUp, Calendar as CalendarIcon, Briefcase } from "lucide-react";

/* ─────────────────────────────────────────────────────────────
   TYPES & INTERFACES
   ───────────────────────────────────────────────────────────── */
type TaskStatus = "Pending" | "In Progress" | "On Hold" | "Completed" | "Cancelled";
...
const getPriorityColor = (priority: TaskPriority) => {
  switch (priority) {
    ...
    case "Low": return "text-base-content/60 bg-base-200 border-base-300";
  }
};
...
export default function MyTasks() {
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      <div className="card bg-base-100 border border-base-200 shadow-sm hover:shadow-md transition-all duration-200">
        <div className="card-body p-4 flex flex-row items-center gap-4">
          <div className="p-3 bg-primary/10 text-primary rounded-xl"><MdSchedule size={24} /></div>
          <div>
            <p className="text-base-content/60 text-xs font-semibold uppercase tracking-wider">Today's Tasks</p>
            <p className="text-2xl font-bold text-base-content">{todayTasks.length}</p>
          </div>
        </div>
      </div>
      <div className="card bg-base-100 border border-base-200 shadow-sm hover:shadow-md transition-all duration-200">
        <div className="card-body p-4 flex flex-row items-center gap-4">
          <div className="p-3 bg-warning/10 text-warning rounded-xl"><MdWarning size={24} /></div>
          <div>
            <p className="text-base-content/60 text-xs font-semibold uppercase tracking-wider">Due Soon</p>
            <p className="text-2xl font-bold text-base-content">{tasks.filter(t => t.status !== "Completed" && t.status !== "Cancelled").length}</p>
          </div>
        </div>
      </div>
      <div className="card bg-base-100 border border-base-200 shadow-sm hover:shadow-md transition-all duration-200">
        <div className="card-body p-4 flex flex-row items-center gap-4">
          <div className="p-3 bg-error/10 text-error rounded-xl"><MdFlag size={24} /></div>
          <div>
            <p className="text-base-content/60 text-xs font-semibold uppercase tracking-wider">Overdue</p>
            <p className="text-2xl font-bold text-base-content">{overdueTasks.length}</p>
          </div>
        </div>
      </div>
      <div className="card bg-base-100 border border-base-200 shadow-sm hover:shadow-md transition-all duration-200">
        <div className="card-body p-4 flex flex-row items-center gap-4">
          <div className="p-3 bg-success/10 text-success rounded-xl"><MdCheckCircle size={24} /></div>
          <div>
            <p className="text-base-content/60 text-xs font-semibold uppercase tracking-wider">Completion</p>
            <div className="flex items-center gap-2">
              <p className="text-2xl font-bold text-base-content">{completionRate}%</p>
              <progress className="progress progress-success w-16" value={completionRate} max="100"></progress>
            </div>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h3 className={`font-semibold text-base-content truncate ${task.status === 'Completed' ? 'line-through text-base-content/50' : ''}`}>
                  {task.title}
                </h3>
                {task.meeting && <MdVideoCall className="text-info shrink-0" size={18} title="Meeting" />}
              </div>
              <div className="flex flex-wrap items-center gap-3 text-xs text-base-content/60">
                <span className="flex items-center gap-1"><Briefcase size={12} /> {task.customer.name}</span>
                <span className="hidden sm:inline">•</span>
                <span className="flex items-center gap-1"><CalendarIcon size={12} /> {formatDate(task.dueDate)}</span>
                <span className="hidden sm:inline">•</span>
                <span className="font-medium text-base-content/80">{formatCurrency(task.expectedDealValue)}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 sm:gap-6 shrink-0 mt-3 sm:mt-0">
              <div className="hidden md:flex flex-col items-end gap-1 w-24">
                ...
                <progress className={`progress w-full h-1.5 ${task.progress === 100 ? 'progress-success' : 'progress-primary'}`} value={task.progress} max="100"></progress>
              </div>
              <span className={`badge badge-sm border font-medium px-2.5 py-3 ${getPriorityColor(task.priority)}`}>
                {task.priority}
              </span>
              <span className={`badge badge-sm border font-medium px-2.5 py-3 ${getStatusColor(task.status)}`}>
                {task.status}
              </span>
            </div>
          </div>
  ...
  const BoardView = () => {
    const columns: TaskStatus[] = ["Pending", "In Progress", "On Hold", "Completed"];
          ...
          >
            <div className="flex items-center justify-between mb-4 px-1">
              <h3 className="font-bold text-base-content text-sm flex items-center gap-2">
                <span className={`w-2.5 h-2.5 rounded-full ${getStatusColor(status).split(' ')[0].replace('text-', 'bg-')}`}></span>
                {status}
              </h3>
              ...
              {filteredTasks.filter(t => t.status === status).map(task => (
                ...
                >
                  <div className="flex justify-between items-start mb-2 gap-2">
                    <span className={`text-[10px] font-bold uppercase tracking-wider ${getPriorityColor(task.priority).split(' ')[0]}`}>
                      {task.priority}
                    </span>
                  ...
                  <p className="text-xs text-base-content/60 mb-3 line-clamp-1">{task.customer.name}</p>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-3 border-t border-base-200/60">
                    ...
                    {task.progress > 0 && task.progress < 100 && (
                      <div className="radial-progress text-primary text-[10px] font-bold bg-base-200 border-none" style={{"--value": task.progress, "--size": "1.5rem", "--thickness": "2px"} as any}></div>
                    )}
                  </div>
                </div>
              ))}
      ...
      <div className="space-y-6 relative before:absolute before:inset-y-0 before:left-4 before:w-0.5 before:bg-base-200">
        {[...filteredTasks].sort((a,b) => new Date(a.dueDate).getTime() - new Date(b.dueDate).getTime()).map(task => (
          <div key={task.id} className="relative pl-10">
            <div className={`absolute left-2.5 w-3.5 h-3.5 rounded-full border-4 border-base-100 top-1.5 ${task.status === 'Completed' ? 'bg-success' : 'bg-primary'}`}></div>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                <p className="text-sm font-bold text-base-content/80">{new Date(task.dueDate).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
                <h4 className="font-semibold text-base-content text-base mt-0.5 cursor-pointer hover:text-primary transition-colors" onClick={() => setSelectedTask(task)}>{task.title}</h4>
                <p className="text-xs text-base-content/60 mt-1">{task.customer.name} • {task.type}</p>
              </div>
              {task.meeting && (
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center py-16 px-4 bg-base-100 rounded-3xl border border-base-200 border-dashed">
      ...
      <h3 className="text-xl font-bold text-base-content mb-2">No tasks found</h3>
      <p className="text-base-content/60 text-center max-w-sm mb-6">You're all caught up! Enjoy your day or check back later for new assignments.</p>
      <button className="btn btn-primary rounded-xl px-8 shadow-sm hover:shadow-md transition-all">Refresh Tasks</button>
    </div>
  );
  ...
  const TaskDrawer = () => {
    ...
    return (
      <>
        {/* Backdrop */}
        <div className="fixed inset-0 bg-base-300/60 backdrop-blur-[2px] z-40 transition-opacity" onClick={() => setSelectedTask(null)}></div>
        ...
        <div className="fixed inset-y-0 right-0 w-full max-w-md bg-base-100 shadow-2xl z-50 flex flex-col transform transition-transform duration-300 border-l border-base-200">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-base-200 bg-base-100/50 backdrop-blur-md">
            <div className="flex items-center gap-3">
              <span className={`badge font-semibold ${getStatusColor(selectedTask.status)} border-none`}>{selectedTask.status}</span>
              <span className="text-xs font-semibold text-base-content/50 uppercase tracking-wider">{selectedTask.type}</span>
            </div>
            <div className="flex items-center gap-2">
              <button className="btn btn-ghost btn-sm btn-circle text-base-content/50 hover:text-base-content"><MdMoreVert size={20}/></button>
              <button onClick={() => setSelectedTask(null)} className="btn btn-ghost btn-sm btn-circle bg-base-200/50 hover:bg-base-300"><MdClose size={20}/></button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
            ...
            {/* Title & Desc */}
            <div>
              <h2 className="text-2xl font-bold text-base-content leading-tight mb-3">{selectedTask.title}</h2>
            ...
            {selectedTask.status !== "Completed" && (
              <div className="bg-base-100 border border-base-200 rounded-2xl p-4 shadow-sm">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-semibold text-base-content">Update Progress</span>
                  <span className="text-xs font-bold text-primary bg-primary/10 px-2 py-1 rounded-lg">{selectedTask.progress}%</span>
                </div>
                <input 
                  type="range" 
            ...
            {/* Customer Quick Card */}
            <div>
              <h3 className="text-xs font-bold uppercase tracking-wider text-base-content/50 mb-3">Customer Information</h3>
                ...
                <div className="card-body p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold text-sm text-base-content">{selectedTask.customer.name}</p>
                        <p className="text-xs text-base-content/60">{selectedTask.customer.region}</p>
                      </div>
                    </div>
                  </div>
                  <div className="space-y-2 bg-base-200/50 p-3 rounded-xl border border-base-200/50">
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <a href={`tel:${selectedTask.customer.phone}`} className="text-base-content/80 hover:text-primary transition-colors font-medium">{selectedTask.customer.phone}</a>
                    </div>
                    <div className="flex items-center gap-3 text-sm">
                      <MdEmail className="text-base-content/40" size={16} />
                      <a href={`mailto:${selectedTask.customer.email}`} className="text-base-content/80 hover:text-primary transition-colors font-medium">{selectedTask.customer.email}</a>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {selectedTask.meeting && (
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-base-content/50 mb-3">Meeting Details</h3>
                <div className="bg-info/5 border border-info/20 rounded-2xl p-4">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2 text-info font-bold">
                      {selectedTask.meeting.mode === "Online" ? <MdVideoCall size={22}/> : <MdLocationOn size={22}/>}
                      <span>{selectedTask.meeting.mode} Meeting</span>
                    </div>
                    <span className="badge badge-info badge-outline font-semibold">{selectedTask.meeting.startTime}</span>
                  </div>
                  
                  {selectedTask.meeting.mode === "Online" && selectedTask.meeting.link && (
                    <a href={selectedTask.meeting.link} target="_blank" rel="noreferrer" className="btn btn-info btn-sm w-full rounded-xl shadow-sm text-info-content">
                      Join Meeting <MdArrowForward />
                    </a>
                  )}
                  {selectedTask.meeting.mode === "Offline" && (
                    <p className="text-sm font-medium text-base-content/80 bg-base-100 p-3 rounded-xl border border-base-200">{selectedTask.meeting.location}</p>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-base-200/40 p-4 rounded-2xl border border-base-200">
                <p className="text-[10px] font-bold uppercase text-base-content/50 mb-1">Expected Value</p>
                <p className="text-lg font-bold text-success">{formatCurrency(selectedTask.expectedDealValue)}</p>
              </div>
              <div className="bg-base-200/40 p-4 rounded-2xl border border-base-200">
                <p className="text-[10px] font-bold uppercase text-base-content/50 mb-1">Probability</p>
                <p className="text-lg font-bold text-primary">{selectedTask.probability || 0}%</p>
              </div>
            </div>
            ...
            {selectedTask.completedNotes && (
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-base-content/50 mb-3 flex items-center gap-2"><MdCheckCircle className="text-success"/> Completion Notes</h3>
      ...
      <div className="fixed inset-0 bg-base-300/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4 animate-fade-in">
        <div className="bg-base-100 rounded-3xl shadow-2xl border border-base-200 w-full max-w-md overflow-hidden transform transition-all">
          <div className="bg-success/10 p-6 flex flex-col items-center border-b border-success/20">
            ...
            <h2 className="text-xl font-bold text-base-content">Complete Task</h2>
            <p className="text-sm font-medium text-success/80 text-center mt-1">"{selectedTask.title}"</p>
          </div>
          <div className="p-6 space-y-4">
            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Completion Notes / Outcome</span></label>
              <textarea 
            ...
            <div className="flex gap-3 pt-2">
              <button onClick={() => setCompleteModalOpen(false)} className="btn btn-ghost flex-1 rounded-xl font-semibold">Cancel</button>
              <button onClick={handleSubmit} className="btn btn-success flex-1 rounded-xl shadow-md text-success-content hover:-translate-y-0.5 transition-transform">Mark Completed</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="fixed inset-0 bg-base-300/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4 animate-fade-in">
        <div className="bg-base-100 rounded-3xl shadow-2xl border border-base-200 w-full max-w-md overflow-hidden transform transition-all">
          <div className="p-6 border-b border-base-200 flex items-center gap-3">
            <div className="p-3 bg-warning/10 text-warning rounded-2xl"><MdSchedule size={24}/></div>
            <div>
              <h2 className="text-lg font-bold text-base-content">Delay Task</h2>
              <p className="text-xs text-base-content/60 font-medium truncate max-w-[250px]">{selectedTask.title}</p>
            </div>
          </div>
          <div className="p-6 space-y-4">
            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">Reason for Delay</span></label>
            ...
            <div className="form-control w-full">
              <label className="label"><span className="label-text font-semibold">New Follow-up Date</span></label>
              <input type="date" className="input input-bordered rounded-xl w-full" />
            </div>
            <div className="flex gap-3 pt-4">
              <button onClick={() => setDelayModalOpen(false)} className="btn btn-ghost flex-1 rounded-xl font-semibold">Cancel</button>
              <button onClick={() => { handleStatusChange(selectedTask.id, "On Hold"); setDelayModalOpen(false); setSelectedTask(null); }} className="btn btn-warning flex-1 rounded-xl shadow-md hover:-translate-y-0.5 transition-transform text-warning-content">Place On Hold</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-base-content tracking-tight flex items-center gap-3">
              Good Morning, John <span className="animate-bounce origin-bottom-right inline-block">👋</span>
            </h1>
            <p className="text-base-content/60 text-sm font-medium mt-1.5">
              Track your assigned tasks, meetings, follow-ups, and sales activities.
            </p>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col xl:flex-row gap-8">
          ...
          <div className="flex-1 w-full space-y-6 min-w-0">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-base-100 p-2 sm:p-3 rounded-2xl border border-base-200 shadow-sm">
              ...
              <div className="relative flex-1 max-w-md group">
                <MdSearch className="absolute left-3.5 top-1/2 -translate-y-1/2 text-base-content/40 group-focus-within:text-primary transition-colors" size={20} />
                <input 
                  type="text" 
              ...
              <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto pb-1 sm:pb-0 hide-scrollbar">
                ...
                <div className="bg-base-200/70 p-1 rounded-xl flex gap-1 shrink-0">
                  ...
                  >
                    <MdViewList size={16}/> <span className="hidden sm:inline">List</span>
                  </button>
                  <button 
                  ...
                  >
                    <MdViewKanban size={16}/> <span className="hidden sm:inline">Board</span>
                  </button>
                  <button 
                  ...
                  >
                    <MdCalendarMonth size={16}/> <span className="hidden sm:inline">Calendar</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="hidden xl:block w-80 shrink-0 space-y-6">
            ...
            <div className="card bg-base-100 border border-base-200 shadow-sm">
              <div className="card-body p-5">
                <h3 className="font-bold text-base-content mb-4 flex items-center gap-2 border-b border-base-200 pb-3">
                  <div className="p-1.5 bg-secondary/10 text-secondary rounded-lg"><MdSchedule size={18}/></div>
                  Today's Agenda
                </h3>
                  ...
                  {todayTasks.length > 0 ? todayTasks.map(t => (
                    <div key={`agenda-${t.id}`} className="flex gap-3">
                      <div className="flex flex-col items-center">
                        <div className={`w-2.5 h-2.5 rounded-full mt-1.5 ${t.status === 'Completed' ? 'bg-success' : 'bg-primary'}`}></div>
                        <div className="w-px h-full bg-base-200 my-1"></div>
                      </div>
                      <div className="pb-3">
                        <p className={`text-sm font-semibold ${t.status === 'Completed' ? 'text-base-content/50 line-through' : 'text-base-content'}`}>{t.title}</p>
                        <p className="text-xs text-base-content/50 mt-0.5">{t.meeting ? t.meeting.startTime : "Anytime today"}</p>
                      </div>
                    </div>
                  )) : (
                    <p className="text-xs text-base-content/50 italic">No tasks scheduled for today.</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="card bg-primary text-primary-content shadow-md overflow-hidden relative">
              <div className="absolute -right-6 -top-6 text-primary-content/10"><TrendingUp size={120} /></div>
              <div className="card-body p-6 relative z-10">
                <h3 className="font-bold text-lg mb-1 opacity-90">Productivity Score</h3>
                <p className="text-4xl font-extrabold mb-4">92<span className="text-xl font-medium opacity-70">/100</span></p>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm font-medium opacity-90">
                    <span>Weekly Goal</span>
                    <span>14/15 Tasks</span>
                  </div>
                  <progress className="progress progress-success bg-primary-content/20 w-full" value="90" max="100"></progress>
                </div>
              </div>
            </div>
