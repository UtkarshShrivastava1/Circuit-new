...
export default function ContactsDashboard() {
  ...
  const stats = useMemo(() => {
    return {
      ...
      active: contacts.filter(c => c.status === "Active" || c.status === "VIP" || c.status === "Customer").length,
      ...
      cell: (info) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-base-content/60">{info.row.original.designation}</div>
          </div>
        </div>
      ),
    }),
    columnHelper.accessor("email", {
      ...
      cell: (info) => (
        <div className="flex flex-col gap-0.5">
          <a href={`mailto:${info.getValue()}`} className="text-sm font-medium text-primary hover:underline flex items-center gap-1"><MdEmail size={14}/> {info.getValue()}</a>
          <span className="text-xs text-base-content/70 flex items-center gap-1"><MdPhone size={14}/> {info.row.original.phoneNumber}</span>
        </div>
      ),
    }),
    columnHelper.accessor("company", {
      header: "Company / Account",
      cell: (info) => <span className="font-semibold text-base-content/90">{info.getValue()}</span>,
    }),
      ...
      cell: ({ row }) => (
        <div className="dropdown dropdown-end" onClick={(e) => e.stopPropagation()}>
          ...
          <ul tabIndex={0} className="dropdown-content z-50 menu p-2 shadow-lg bg-base-100 rounded-box w-48 border border-base-200">
            <li><a onClick={() => navigate(`/sales/contacts/${row.original.id}`)}><MdPerson size={16} /> View Profile</a></li>
            <li><a onClick={() => navigate(`/sales/contacts/edit/${row.original.id}`)}><MdEdit size={16} /> Edit Contact</a></li>
  ...
  return (
    <div className="min-h-screen bg-base-200 p-4 md:p-6 font-sans flex flex-col h-full overflow-hidden relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 bg-base-100 p-5 rounded-xl border border-base-300 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-base-content tracking-tight">Contacts Management</h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={() => navigate("/sales/contacts/new")} className="btn btn-primary btn-sm gap-2 shadow-sm">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Total Contacts", value: stats.total, color: "text-base-content" },
          { label: "Active & VIP", value: stats.active, color: "text-primary" },
          { label: "New Prospects", value: stats.prospects, color: "text-info" },
          { label: "Linked Leads", value: stats.linkedLeads, color: "text-success" },
        ].map((stat, idx) => (
      ...
      {showFilters && (
        <div className="bg-base-100 border border-base-300 rounded-xl p-5 mb-4 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 shadow-sm animate-fade-in-down">
          ...
          <div>
            <label className="text-xs font-bold text-base-content/70 mb-1 block uppercase">Contact Source</label>
            <select className="select select-sm select-bordered w-full"><option>All</option><option>Website</option><option>Cold Call</option><option>Referral</option></select>
          </div>
          <div>
            <label className="text-xs font-bold text-base-content/70 mb-1 block uppercase">Assigned Rep</label>
      ...
      {Object.keys(rowSelection).length > 0 && (
        <div className="bg-primary/10 border border-primary/20 rounded-xl p-3 mb-4 flex items-center justify-between shadow-sm animate-fade-in-up">
          <span className="text-sm font-semibold text-primary">{Object.keys(rowSelection).length} contacts selected</span>
          <div className="flex gap-2">
            ...
            <button className="btn btn-xs btn-outline bg-base-100">Send Email</button>
      ...
      <div className="flex-1 bg-base-100 border border-base-300 rounded-xl overflow-hidden shadow-sm flex flex-col relative">
                ...
                <tr>
                  <td colSpan={columns.length} className="text-center py-16">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-base-content/50 font-medium">No contacts found matching your criteria.</p>
                      <button onClick={() => navigate("/sales/contacts/new")} className="btn btn-outline btn-sm mt-2">Create New Contact</button>
                    </div>
                  </td>
                </tr>
              )}
        ...
        <div className="border-t border-base-300 p-3 bg-base-100 flex items-center justify-between text-sm">
          <span className="text-base-content/60 font-medium">
            Showing {table.getRowModel().rows.length} of {filteredContacts.length} contacts
          </span>
          <div className="flex items-center gap-3">
            <select 
