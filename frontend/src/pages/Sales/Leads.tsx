...
const leadSchema = z.object({
  ...
  status: z.enum(["New", "Contacted", "Qualified", "Proposal Sent", "Negotiation", "Won", "Lost"]).default("New"),
  ...
  return (
    <div className="min-h-screen bg-base-200 p-4 md:p-6 lg:p-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 bg-base-100 p-5 rounded-xl border border-base-300 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-base-content tracking-tight">Create New Lead</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
            ...
            <div className="collapse-content pt-5 space-y-4">
              ...
              <FormRow label="Phone Number" required error={errors.phoneNumber?.message}>
                <div className={`flex border rounded-lg overflow-hidden ${errors.phoneNumber ? 'border-error' : 'border-base-300'} focus-within:border-primary transition-colors bg-base-100`}>
                  ...
                  <input type="tel" {...register("phoneNumber")} className="input input-sm border-none w-full focus:outline-none" placeholder="81234 56789" />
                </div>
              </FormRow>
            </div>
          </div>
          ...
          <div className="collapse collapse-arrow bg-base-100 border border-base-300 rounded-xl shadow-sm">
            ...
            <div className="collapse-content pt-5 space-y-4">
              <FormRow label="Company Name" required error={errors.companyName?.message}>
                <input {...register("companyName")} className={`input input-bordered w-full ${errors.companyName ? 'input-error' : ''}`} placeholder="Company Inc." />
              </FormRow>
          ...
          <div className="collapse collapse-arrow bg-base-100 border border-base-300 rounded-xl shadow-sm">
            ...
            <div className="collapse-content pt-5">
              <FormRow label="Description">
                <textarea {...register("description")} className="textarea textarea-bordered w-full bg-warning/5" rows={4} placeholder="Internal notes and lead requirements..."></textarea>
              </FormRow>
            </div>
          </div>
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-base-100 border border-base-300 rounded-xl p-5 sticky top-24 shadow-sm">
            <h3 className="font-bold text-lg mb-4 pb-2 border-b border-base-200">Lead Summary</h3>
            
            <div className="space-y-4">
              <div>
                <span className="text-xs text-base-content/60 uppercase font-semibold">Status & Priority</span>
                <div className="mt-1 flex gap-2">
                  <div className={`badge font-bold ${wStatus === 'New' ? 'badge-info' : 'badge-primary'}`}>{wStatus || "New"}</div>
                  <div className={`badge badge-outline font-bold ${wPriority === 'Urgent' ? 'badge-error' : wPriority === 'High' ? 'badge-warning' : ''}`}>{wPriority || "Medium"}</div>
                </div>
              </div>

              <div>
                <span className="text-xs text-base-content/60 uppercase font-semibold">Lead Name</span>
                <p className="font-medium text-base-content mt-1 truncate">{wFirstName || wLastName ? `${wFirstName || ''} ${wLastName || ''}` : "—"}</p>
              </div>
              ...
              <div>
                <span className="text-xs text-base-content/60 uppercase font-semibold">Lead Owner</span>
            ...
            <div className="mt-6 pt-4 border-t border-base-200 space-y-2">
              <button type="submit" disabled={isSubmitting} className="btn btn-primary w-full shadow-sm">
                {isSubmitting ? <span className="loading loading-spinner loading-sm"></span> : <><MdSave size={18} /> Save Lead</>}
              </button>
