...
>>>>>>> origin/Ritika
  ...
  const tasks = useMemo(() => {
    return (data?.data || []).map((t: any) => ({
      ...
      followUpDate: t.followUpDate
        ? new Date(t.followUpDate).toISOString().split("T")[0]
        : "",
  ...
  const updateMutation = useMutation({
    mutationFn: (vars: { id: string; payload: Partial<any> }) =>
      updateSalesTask(vars.id, vars.payload, auth.slug || "default-tenant"),
    onSuccess: () =>
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      deleteSalesTask(id, auth.slug || "default-tenant"),
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["salesTasks"] }),
  });
      ...
      columnHelper.display({
        ...
        cell: ({ row }) => (
          ...
          >
            ...
            >
              <li>
                <a onClick={() => setSelectedTask(row.original)}>
                  <MdViewList size={16} /> View Details
                </a>
              </li>
              <li>
                <a
...
>>>>>>> origin/Ritika
    ...
    try {
      await Promise.all(
        selected.map((t) =>
          updateMutation.mutateAsync({
            id: t.id,
            payload: { assignedTo: newAssignee },
          }),
        ),
      );
  ...
  const handlePostComment = async () => {
    ...
    try {
      const updatedNotes = selectedTask.completionNotes
        ? `${selectedTask.completionNotes}\n- ${newComment}`
        : `- ${newComment}`;
      await updateMutation.mutateAsync({
        id: selectedTask.id,
        payload: { completionNotes: updatedNotes },
      });
  ...
  return (
    ...
    <div className="min-h-screen bg-base-200 p-4 md:p-6 font-sans flex flex-col overflow-x-hidden">
      {/* ── Header ── */}
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-6 gap-4 bg-base-100 p-4 md:p-5 rounded-xl border border-base-300 shadow-sm">
        {/* Left Section */}
        <div className="min-w-0">
          <h1 className="text-lg md:text-xl font-bold text-base-content tracking-tight">
            Sales Tasks
          </h1>
          ...
          <div className="mt-1 text-[13px] text-base-content/60 flex flex-wrap items-center gap-y-1">
            ...
            <span>Sales</span>
            <MdChevronRight className="text-primary mx-1 text-base-content/50" />
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full lg:w-auto">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <button
              className="btn btn-outline btn-sm btn-square flex-1 sm:flex-none"
              onClick={() => refetch()}
            >
              <MdRefresh size={16} />
            </button>
            ...
            <button
              ...
              className="btn btn-primary btn-sm gap-2 flex-1 sm:flex-none whitespace-nowrap"
            >
      ...
      "
          >
            ...
            <span className="text-[10px] sm:text-xs text-base-content/60 mt-1 font-medium uppercase leading-tight">
              {stat.label}
            </span>
          </div>
        ))}
      ...
      <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4 mb-4 bg-base-100 p-3 rounded-xl border border-base-300 shadow-sm">
        ...
        {/* Right: View Switch */}
        <div className="flex bg-base-200 p-1 rounded-lg border border-base-300 w-full lg:w-auto overflow-x-auto">
          <button
      ...
      {Object.keys(rowSelection).length > 0 && view === "table" && (
        <div className="bg-primary/10 border border-primary/20 rounded-xl p-3 mb-4 flex items-center justify-between shadow-sm animate-fade-in-up">
          <span className="text-sm font-semibold text-primary">
            {Object.keys(rowSelection).length} tasks selected
          </span>
          <div className="flex gap-2">
            <button
            ...
            >
              <MdAssignmentInd /> Assign Employee
            </button>
            <button
      ...
      {isLoading ? (
        ...
        // <div className="w-full bg-base-100 border border-base-300 rounded-xl shadow-sm flex flex-col min-h-[500px]">
        <div className="w-full bg-base-100 border border-base-300 rounded-xl shadow-sm flex flex-col flex-1 min-h-0">
          {/* View 1: Table */}
          {view === "table" && (
            <div className="w-full overflow-x-auto">
              <table className="table table-sm w-full min-w-[900px] text-sm">
                <thead>
...
=======
          ...
          {view === "kanban" && (
            <div className="flex-1 flex overflow-x-auto p-4 gap-4 bg-base-200/30">
              {(
                ["Pending", "In Progress", "On Hold", "Completed"] as const
              ).map((status) => (
                <div
                  key={status}
                      ...
                      .map((task) => (
                        ...
                        >
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <span
                              className={`badge badge-xs ${task.priority === "Urgent" ? "badge-error" : task.priority === "High" ? "badge-warning" : "badge-neutral"}`}
                            >
                              {task.priority}
                            </span>
                          </div>
                          ...
                          <div className="flex justify-between items-center mt-2 border-t border-base-200 pt-2">
                            ...
                            <span
                              className={`text-[10px] font-semibold ${task.dueDate < new Date().toISOString().split("T")[0] ? "text-error" : "text-base-content/60"}`}
                            >
                              Due: {task.dueDate}
                            </span>
>>>>>>> origin/Ritika
          ...
          {view === "calendar" && (
            <div className="flex-1 p-6 flex items-center justify-center bg-base-200/30">
              <div className="text-center space-y-4">
                ...
                />
                <h3 className="text-lg font-bold text-base-content/50">
                  Calendar View
                </h3>
                <p className="text-sm text-base-content/40 max-w-sm">
                  Full calendar integration requires a library like
                  react-big-calendar. Tasks are mapped to due dates here.
                </p>
              </div>
            </div>
          )}
          ...
          {view === "table" && (
            <div className="border-t border-base-300 p-3 bg-base-100 flex flex-col sm:flex-row gap-3 sm:gap-0 sm:items-center sm:justify-between text-sm">
              <span className="text-base-content/60 text-center sm:text-left">
                Showing {table.getRowModel().rows.length} of{" "}
                {filteredTasks.length} tasks
              </span>
              <div className="flex flex-col xs:flex-row sm:flex-row items-center gap-2 w-full sm:w-auto">
                <select
      ...
      {/* ── Task Details Drawer ── */}
      <div
        className={`fixed inset-0 bg-black/40 z-50 transition-opacity ${selectedTask ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
      >
        <div
          className={`absolute right-0 top-0 h-full w-full md:w-[600px] bg-base-100 shadow-2xl transition-transform duration-300 transform ${selectedTask ? "translate-x-0" : "translate-x-full"} flex flex-col`}
        >
          {/* Drawer Header */}
          <div className="p-5 border-b border-base-300 flex justify-between items-center bg-base-200/50">
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <span
                  className={`badge badge-sm badge-outline ${selectedTask?.status === "Completed" ? "badge-success" : "badge-primary"}`}
                >
                  {selectedTask?.status}
                </span>
              </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-base-100">
            ...
            <div className="flex gap-2 pb-6 border-b border-base-200">
              <button
                className="btn btn-sm btn-success text-white flex-1 gap-2"
                onClick={() =>
                  selectedTask &&
                  handleStatusChange(selectedTask.id, "Completed")
                }
              >
                <MdOutlineCheckCircle /> Mark Complete
              </button>
              <button
              ...
              <button
                className="btn btn-sm btn-outline flex-1"
                onClick={() => {
                  setNewFollowUpDate(selectedTask?.followUpDate || "");
...
=======
                ...
                <div>
                  <p className="text-base-content/50 mb-1">Task Type</p>
                  <p className="font-semibold">{selectedTask?.type}</p>
                </div>
                <div>
                  <p className="text-base-content/50 mb-1">Assigned To</p>
                  <p className="font-semibold">{selectedTask?.assignedTo}</p>
                </div>
                <div>
                  <p className="text-base-content/50 mb-1">Priority</p>
                  <span
                    className={`badge badge-sm font-semibold ${selectedTask?.priority === "Urgent" ? "badge-error" : "badge-warning"}`}
                  >
...
>>>>>>> origin/Ritika
            ...
            <section>
              ...
              <div className="bg-base-200/50 p-4 rounded-xl border border-base-200 grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="text-base-content/50 mb-1">Deal Value</p>
                  <p className="font-bold text-success text-lg">
                    ₹{selectedTask?.dealValue.toLocaleString()}
                  </p>
                </div>
                <div>
                  <p className="text-base-content/50 mb-1">Stage</p>
                  <p className="font-semibold">{selectedTask?.stage}</p>
                </div>
                <div className="col-span-2 divider my-0"></div>
                <div>
                  <p className="text-base-content/50 mb-1">Start Date</p>
                  <p className="font-medium">{selectedTask?.startDate}</p>
                </div>
                <div>
                  <p className="text-base-content/50 mb-1">Due Date</p>
                  <p className="font-medium">{selectedTask?.dueDate}</p>
                </div>
                <div>
                  <p className="text-base-content/50 mb-1">Follow-up Date</p>
      ...
      <dialog className={`modal ${bulkStatusModalOpen ? "modal-open" : ""}`}>
        <div className="modal-box">
          ...
          <select
            ...
            value={newStatus}
            onChange={(e) =>
              setNewStatus(e.target.value as SalesTask["status"])
            }
          >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="form-control">
                  ...
                  <input
                    type="text"
                    className="input input-bordered w-full"
                    value={taskToEdit.customer}
                    onChange={(e) =>
                      setTaskToEdit({ ...taskToEdit, customer: e.target.value })
                    }
                    required
                  />
                </div>
...
=======
                    ...
                    <ul
                      tabIndex={0}
                      className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-full max-h-60 overflow-y-auto z-50 border border-base-300"
                    >
                      {salesReps.map((rep) => (
                        <li key={rep}>
                          <a
                            onClick={() => {
                              setTaskToEdit({ ...taskToEdit, assignedTo: rep });
                              (document.activeElement as HTMLElement)?.blur();
                            }}
                          >
                            {rep}
...
>>>>>>> origin/Ritika
                ...
                <div className="form-control">
                  <label className="label">
                    <span className="label-text">Opportunity Stage</span>
                  </label>
                  <select
                    className="select select-bordered w-full"
                    value={taskToEdit.stage}
                    onChange={(e) =>
                      setTaskToEdit({ ...taskToEdit, stage: e.target.value })
                    }
                  >
                ...
                <div className="form-control">
                  <label className="label">
                    <span className="label-text">Deal Value (₹)</span>
                  </label>
                  <input
                    type="number"
                    className="input input-bordered w-full"
                    value={taskToEdit.dealValue}
                    onChange={(e) =>
                      setTaskToEdit({
                        ...taskToEdit,
