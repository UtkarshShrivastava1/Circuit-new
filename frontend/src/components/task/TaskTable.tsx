...
function TaskTable({ tasks, onOpenTask, onEditTask, onDeleteTask }: Props) {
  ...
  return (
    <div className="bg-base-100 border border-base-300 rounded-lg overflow-x-scroll">
      <table className="table table-sm table-zebra w-full min-w-150 ">
        <thead className=" bg-primary text-sm text-primary-content border-primary/60">
        ...
        <tbody>
          {tasks.map((task) => (
            <tr
              key={task._id}
              className="divide-x divide-purple-50 cursor-pointer hover:bg-base-200 text-base-content text-[12.5px]"
